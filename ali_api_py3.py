...
# -*- coding:utf-8 -*-
# ########################################
# Function:    演示阿里云裸API使用方法 增加多服务支持 PY3
# Example Usage:
#   python3 ali_api_py3.py Service=cdn Action=DescribeCdnDomainLogs DomainName=appcdn.example.com LogDay=2018-05-20 PageSize=1000
#   python3 ali_api_py3.py Service=slb Action=DescribeLoadBalancers RegionId=cn-qingdao
#   python3 ali_api_py3.py Service=ecs Action=DescribeInstances RegionId=cn-qingdao
# ########################################
...
def percent_encode(str):
    res = parse.quote(str.encode('utf8'), '')
    res = res.replace('+', '%20')
    res = res.replace('*', '%2A')
    res = res.replace('%7E', '~')
    return res
...
def compose_params(service, user_params):
    ...
    # api version different between service:
...
def make_request(service, user_params, quiet=False):
    params = compose_params(service, user_params)
    service_url = service_address.format(service)
    print(("URL: {}/?{}".format(service_url, parse.urlencode(params))))
...
if __name__ == '__main__':
    parser = OptionParser("%s Service=service Action=action Param1=Value1 Param2=Value2\n" % sys.argv[0])
